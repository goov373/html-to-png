 ...
 */
function toHtmlToImageOptions(options = {}, isJpeg = false) {
    return {
 ...
 * ```
 */
export async function exportToJpeg(element, options = {}) {
    if (!element) {
 ...
 * ```
 */
export async function exportToBlob(element, options = {}) {
    if (!element) {
 ...
 * ```
 */
export async function exportToCanvas(element, options = {}) {
    if (!element) {
 ...
 * ```
 */
export async function captureViewport(container, options = {}) {
    if (!container) {
        throw new Error('Container element is required for viewport capture');
    }
    const { format = 'png', hideScrollbars = true, region, pixelRatio = DEFAULT_PIXEL_RATIO, ...restOptions } = options;
    ...
    try {
        ...
        if (region) {
            ...
            ctx.drawImage(fullCanvas, scaledRegion.x, scaledRegion.y, scaledRegion.width, scaledRegion.height, 0, 0, scaledRegion.width, scaledRegion.height);
 ...
 * ```
 */
export function getViewportInfo(container) {
    if (!container) {
